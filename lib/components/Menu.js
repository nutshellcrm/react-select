...
function getMenuPlacement(_ref) {
  ...
  var _menuEl$getBoundingCl = menuEl.getBoundingClientRect(),
      ...
      menuClientTop = _menuEl$getBoundingCl.top; // If not fixed position, need to possibly adjust based on scroll parent


  var menuTop = isFixedPosition ? menuClientTop : menuClientTop - scrollParentTop;

  var _menuEl$offsetParent$ = menuEl.offsetParent.getBoundingClientRect(),
      containerTop = _menuEl$offsetParent$.top;

  var viewHeight = isFixedPosition ? window.innerHeight : scrollHeight;
  ...
  switch (placement) {
    ...
    case 'bottom':
      ...
      if (placement === 'bottom') {
        (0, _utils.scrollTo)(scrollParent, scrollDown); // We might also need to constrain the height

        var _constrainedHeight2 = maxHeight;

        if (maxHeight > scrollHeight && !isFixedPosition) {
          _constrainedHeight2 = scrollHeight - marginTop - marginBottom;
        }
    ...
    case 'top':
      ...
      if (!isFixedPosition && scrollSpaceAbove >= minHeight || isFixedPosition && viewSpaceAbove >= minHeight) {
        var _constrainedHeight3 = maxHeight; // we want to provide as much of the menu as possible to the user,
        // so give them whatever is available below rather than the minHeight.

        if (!isFixedPosition && scrollSpaceAbove >= minHeight || isFixedPosition && viewSpaceAbove >= minHeight) {
          _constrainedHeight3 = isFixedPosition ? viewSpaceAbove - marginTop : scrollSpaceAbove - marginTop;
        }
